...
export default function SubcategoryMenu({ category, subcategories, visible, color }: SubcategoryMenuProps) {
  ...
  return (
    <div className="w-64 h-[500px] bg-white rounded-xl shadow-2xl border border-gray-100 p-4 animate-in slide-in-from-left-2 duration-300 flex flex-col">
      <div className="mb-6">
        ...
        <div className={`h-0.5 ${colorClasses[color].accent} w-8 mt-1 transition-all duration-500`} />
      </div>
      ...
      <div className="flex-grow flex flex-col">
        ...
        <div className="mt-auto pt-4 border-t border-gray-100">
          <button className={`w-full text-sm ${colorClasses[color].text} hover:${colorClasses[color].accent} hover:text-white px-3 py-2 rounded-lg transition-all duration-300 font-medium hover:shadow-md transform hover:scale-105`}>
            View All {category}
          </button>
        </div>
      </div>
